...
###########################################################################################
class XfyunTTS(BaseTTS):
    def __init__(self, opt, parent):
        super().__init__(opt, parent)
        # 从配置中读取讯飞参数
        self.appid = "5f2d491c"
        self.api_key = "1ecd1a27b1d944b23313abd9642352f6"
        self.api_secret = "4a62a7b3e1b3ed39295501fea873aac6"
        self.host = "ws-api.xfyun.cn"
        self.base_url = f"wss://tts-api.xfyun.cn/v2/tts"

        # 公共参数 (common)
        self.common_args = {"app_id": self.appid}

        # 业务参数 (business)：调整为 raw PCM 以支持流式输出
        self.business_args = {
            "aue": "raw",  # 输出为 raw PCM (非 lame/MP3)
            "sfl": 1,  # base64 编码返回
            "auf": "audio/L16;rate=8000",  # 16kHz, 16bit PCM
            "vcn": "x4_xiaoyan",  # 发音人
            "tte": "utf8"  # 文本编码
        }
    ...
    async def xfyun_voice(self, text):  # -> Iterator[bytes]:
        """
        执行讯飞 TTS 语音合成，返回 PCM 音频字节流迭代器。
        """
        logger.info(f"Starting xfyun_voice for text: {text}")
        start = time.perf_counter()

        try:
            url, request_json = self._create_url(text)
            first = True

            async with websockets.connect(url, ping_interval=None) as ws:
                # 发送请求 JSON
                await ws.send(json.dumps(request_json))
                logger.info("------> Sent text data to Xfyun TTS")
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.debug(f"Received message: code={code}, status={status}")
                        ...
                        audio = base64.b64decode(audio_b64)  # raw PCM bytes

                        if status == 2:  # 最后一帧
                            if first:
                                end = time.perf_counter()
                                logger.info(f"Xfyun TTS Time to first chunk: {end - start}s")
                                first = False
                            yield audio
                            logger.info("ws is closed (last frame)")
                            break
                        else:
                            if first:
                                end = time.perf_counter()
            ...
            logger.exception('Xfyun TTS error')
            # 可选：yield 空字节或 raise

    def txt_to_audio(self, msg):
        """
        同步调用异步 TTS 方法，处理文本到音频。
        """
        text, textevent = msg
        asyncio.new_event_loop().run_until_complete(
            self.stream_tts(
                self.xfyun_voice(text),
                msg
            )
        )
    ...
    async def stream_tts(self, audio_stream, msg):
        """
        流式处理 TTS 音频流，转换为 float32 PCM chunks 并发送到 parent。
        假设 self.sample_rate = 16000, self.chunk = 适当大小 (e.g., 1024 frames)。
        """
        text, textevent = msg
        first = True
        all_audio = []  # 收集所有 PCM 数据 (可选，用于调试)
        last_stream = np.array([], dtype=np.float32)

        async for chunk in audio_stream:
            if chunk is not None and len(chunk) > 0:
                # chunk 是 raw L16 (int16) PCM
                ...
                while streamlen >= self.chunk:
                    ...
                    if first:
                        ...
                        first = False
                    self.parent.put_audio_frame(stream[idx:idx + self.chunk], eventpoint)
                    all_audio.extend(stream[idx:idx + self.chunk])
                    streamlen -= self.chunk
                    idx += self.chunk
                last_stream = stream[idx:]  # 剩余流

        # 发送结束事件
        eventpoint = {'status': 'end', 'text': text, 'msgenvent': textevent}
        self.parent.put_audio_frame(np.zeros(self.chunk, np.float32), eventpoint)

        # 可选：收集所有音频并保存为 WAV (调试用)
        # all_audio.extend(np.zeros(self.chunk, np.float32))
        # full_audio = np.array(all_audio, dtype=np.float32)
        # output_file = "xfyun_output.wav"
        # import soundfile as sf
        # sf.write(output_file, full_audio, self.sample_rate)
        # logger.info(f"Audio saved to: {output_file} (length: {len(full_audio)/self.sample_rate:.2f}s)")
###########################################################################################
...
class DoubaoTTS(BaseTTS):
    def __init__(self, opt, parent):
        ...
        self.appid="1174875031"
        self.token="SKYpgl0ntL-Frs_FETKTwComdyTajzTv"
    ...
    async def doubao_voice(self, text): # -> Iterator[bytes]:
        logger.exception(f"test doubao_voice{text}")
        start = time.perf_counter()
    ...
    async def stream_tts(self, audio_stream, msg):
        ...
        # sf.write(output_file, full_audio, self.sample_rate)
        # print(f"音频保存到: {output_file} (长度: {len(full_audio)/self.sample_rate:.2f}s)")
